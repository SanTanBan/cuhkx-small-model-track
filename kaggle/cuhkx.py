...
_DEPTH_IR_PC = (("Depth_Color_PC", 3), ("IR_PC", 1))
# DepthIR_KD / DepthIR_KDT are the same input under other names, so distilled
# students (train-only / train + test soft targets) get their own tags and logs
# when they train side by side in one session; DepthIR_R3A / DepthIR_R3B do the
# same for two round-3 self-trained students (144 / 128 px).
COMPOSITES = {"DepthIR_PC": _DEPTH_IR_PC, "DepthIR_KD": _DEPTH_IR_PC, "DepthIR_KDT": _DEPTH_IR_PC,
              "DepthIR_R3A": _DEPTH_IR_PC, "DepthIR_R3B": _DEPTH_IR_PC}
...
class _Res1D(nn.Module):
    def __init__(self, cin, cout, stride):
        ...
        self.body = nn.Sequential(
            nn.Conv1d(cin, cout, 5, stride, 2, bias=False), nn.BatchNorm1d(cout),
            nn.ReLU(inplace=True),
            nn.Conv1d(cout, cout, 5, 1, 2, bias=False), nn.BatchNorm1d(cout))
...
class IMUNet(nn.Module):
    """
    1-D ResNet over the five body-worn IMUs (both arms, chest, both legs):
    acceleration and angular velocity per device resampled to T steps, plus a
    presence channel per device (prep.encode_imu). The arm sensors carry the
    wrist motion that separates writing, typing and phone use, which the
    cameras resolve as a few pixels. Input normalisation lives in buffers, so
    it ships with the weights.
    """

    def __init__(self, in_ch, num_classes=NUM_CLASSES, width=48, dropout=0.3):
        super().__init__()
        self.register_buffer("mu", torch.zeros(1, in_ch, 1))
        self.register_buffer("sd", torch.ones(1, in_ch, 1))
        w = width
        self.stem = nn.Sequential(nn.Conv1d(in_ch, w, 7, 1, 3, bias=False),
                                  nn.BatchNorm1d(w), nn.ReLU(inplace=True))
        self.blocks = nn.Sequential(_Res1D(w, w, 1), _Res1D(w, 2 * w, 2),
                                    _Res1D(2 * w, 2 * w, 1), _Res1D(2 * w, 4 * w, 2),
                                    _Res1D(4 * w, 4 * w, 1))
        self.drop = nn.Dropout(dropout)
        self.fc = nn.Linear(8 * w, num_classes)

    def forward(self, x):                        # x: [B, C, T]
        f = self.blocks(self.stem((x - self.mu) / self.sd))
        return self.fc(self.drop(torch.cat([f.mean(-1), f.amax(-1)], 1)))


def make_model(cfg: Cfg, pretrained=True):
    """Pose -> graph net; IMU -> 1-D ResNet; images -> a 3D video net if cfg.arch
    names one, else TSM 2D."""
    if cfg.modality.upper().startswith("IMU"):
        return IMUNet(int(cfg.extra.get("imu_ch", 35)),
                      width=int(cfg.extra.get("imu_width", 48)), dropout=cfg.dropout)
...
CLIP_RATIOS = (1.0, 0.95, 0.9, 0.85, 0.8, 0.75, 0.7, 0.65, 0.6, 0.55, 0.5)


def _channel_scale(w, qmax):
    """
    Per-output-channel scale that minimises the squared weight error, searched
    over clipping ratios of the channel's max |w|. Max-abs scaling spends the
    few integer levels on rare outliers: at 6 bits it flipped 4 of 32
    R(2+1)D-34 test predictions, where 8 bits flipped 1. Scales are rounded to
    fp16 before use, since that is what the checkpoint stores.
    """
    flat = w.reshape(w.shape[0], -1)
    amax = flat.abs().amax(1).clamp_min(1e-6)
    best = torch.full_like(amax, float("inf"))
    scale = (amax / qmax).half().float().clamp_min(1e-6)
    for r in CLIP_RATIOS:
        s = (amax * r / qmax).half().float().clamp_min(1e-6)
        q = (flat / s[:, None]).round().clamp(-qmax, qmax)
        err = ((q * s[:, None] - flat) ** 2).sum(1)
        better = err < best
        best = torch.where(better, err, best)
        scale = torch.where(better, s, scale)
    return scale.view(-1, *([1] * (w.ndim - 1)))


def quantize_int8(sd, min_numel=4096, bits=8):
    """
    Per-output-channel symmetric integer conv/linear weights with an fp16
    scale; every other tensor fp16. bits=8 stores int8 (R(2+1)D-34, 63.5 M
    params: 127 MB in fp16 -> ~64 MB); fewer bits are bit-packed (6 bits ->
    ~48 MB). Weights are divided by the fp16-rounded scale, so dequantize()
    reproduces exactly the values that were evaluated.
    """
    qmax = (1 << (bits - 1)) - 1
    out = {}
    for k, v in sd.items():
        if v.is_floating_point() and v.ndim >= 2 and v.numel() >= min_numel:
            w = v.float()
            scale = _channel_scale(w, qmax).half()
            q = (w / scale.float()).round().clamp(-qmax, qmax).to(torch.int8)
            out[k] = ({"q": q, "scale": scale} if bits == 8 else
                      {"packed": _pack_bits(q, bits), "shape": list(q.shape), "bits": bits,
                       "scale": scale})
        else:
...
def _pack_bits(q, bits):
    """Signed ints with |q| < 2^(bits-1) -> uint8 tensor carrying `bits` bits per value."""
    u = q.flatten().numpy().astype(np.int16) & ((1 << bits) - 1)       # two's complement
    b = ((u[:, None] >> np.arange(bits, dtype=np.int16)) & 1).astype(np.uint8)
    return torch.from_numpy(np.packbits(b.reshape(-1), bitorder="little"))


def _unpack_bits(packed, shape, bits):
    n = int(np.prod(shape))
    b = np.unpackbits(packed.numpy(), bitorder="little")[: n * bits].reshape(n, bits)
    u = (b.astype(np.int16) << np.arange(bits, dtype=np.int16)).sum(1)
    u = np.where(u >= (1 << (bits - 1)), u - (1 << bits), u)
    return torch.from_numpy(u.astype(np.int8).reshape(shape))


def dequantize(sd):
    """Inverse of quantize_int8, any bit width (also accepts plain fp16/fp32 state dicts)."""
    out = {}
    for k, v in sd.items():
        if isinstance(v, dict):
            q = v["q"] if "q" in v else _unpack_bits(v["packed"], v["shape"], v["bits"])
            out[k] = q.float() * v["scale"].float()
        else:
            out[k] = v.float() if v.is_floating_point() else v
    return out
