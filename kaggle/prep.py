...
# Person detection: torchvision's COCO SSDlite320-MobileNetV3 on a few IR
# frames per clip. The Kaggle detection kernel (training crops) and infer.py
# (test crops) share every step below, so the two cannot drift apart.
DET_FRAMES, DET_QUALITY, DET_SCORE = 4, 85, 0.30


def det_jpegs(src, T=DET_FRAMES, quality=DET_QUALITY):
    """
    Up to T uniformly spaced frames of a clip as grayscale JPEGs at native
    resolution: the detector's input. Returns (jpeg_bytes, (h, w)), or None
    when no frame decodes.
    """
    frames, get = _frame_source(src)
    if not frames:
        return None
    out, shape = [], None
    for j in sorted(set(sample_idx(len(frames), T))):
        try:
            im = get(frames[j])
        except Exception:
            im = None
        if im is None:
            continue
        if im.ndim == 3:
            im = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)       # IR is single-channel
        ok, buf = cv2.imencode(".jpg", im, [int(cv2.IMWRITE_JPEG_QUALITY), quality])
        if ok:
            out.append(buf.tobytes())
            shape = im.shape[:2]
    return (out, shape) if out else None


def det_input(buf):
    """One detector JPEG -> [3,H,W] float tensor in [0,1], or None."""
    import torch
    img = cv2.imdecode(np.frombuffer(buf, np.uint8), cv2.IMREAD_GRAYSCALE)
    if img is None:
        return None
    rgb = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)
    return torch.from_numpy(rgb).permute(2, 0, 1).float() / 255


def best_person(out, score=DET_SCORE):
    """Highest-scoring COCO person (label 1) at or above `score`, as
    (x0, y0, x1, y1, score); None if there is none."""
    import torch
    keep = (out["labels"] == 1) & (out["scores"] >= score)
    if not bool(keep.any()):
        return None
    k = int(torch.argmax(out["scores"] * keep))
    x0, y0, x1, y1 = (float(v) for v in out["boxes"][k].tolist())
    return x0, y0, x1, y1, float(out["scores"][k])


def make_detector(state_dict=None):
    """
    SSDlite320-MobileNetV3 (COCO, BSD-3 torchvision), weights used exactly as
    given (fp32). The packaged checkpoint stores the detector unconverted, so
    test crops come from the very network that made the training crops.
    """
    from torchvision.models import detection
    net = detection.ssdlite320_mobilenet_v3_large(weights=None, weights_backbone=None)
...
IMU_DEVICES = ("LA", "RA", "C", "LL", "RL")    # left/right arm, chest, left/right leg
IMU_T = 40                                       # ~2 s clips at ~11 Hz: ~20 rows/device


def _imu_seconds(stamp):
    """
    '2025-6-17 14:34:47.47' -> seconds of the day. The fraction is an integer
    millisecond count without zero padding ('.47' = 47 ms, '.70' = 70 ms,
    '.436' = 436 ms), so it is not read as a decimal fraction.
    """
    hms, _, frac = str(stamp).strip().split(" ")[-1].partition(".")
    h, m, s = hms.split(":")
    return int(h) * 3600 + int(m) * 60 + int(s) + (int(frac) if frac.isdigit() else 0) / 1000.0


def parse_imu_csv(buf):
    """
    One IMU CSV -> {device: (t [n], acc xyz + gyro xyz [n, 6])}, rows sorted by
    time (files interleave devices slightly out of order). Training files carry
    Chinese headers and test files English ones in the same column order, so
    columns are taken by position: time, device, acceleration (g), angular
    velocity (deg/s). Device codes come from names like 'WTLA(D2:5B:...)'.
    """
    import io
    import re
    try:
        df = pd.read_csv(io.BytesIO(buf), encoding="utf-8", encoding_errors="replace")
    except Exception:
        return {}
    if df.shape[1] < 8 or not len(df):
        return {}
    t = df.iloc[:, 0].map(_imu_seconds).to_numpy(np.float64)
    sig = df.iloc[:, 2:8].apply(pd.to_numeric, errors="coerce").to_numpy(np.float64)
    out = {}
    for dev, idx in df.groupby(df.columns[1]).indices.items():
        m = re.match(r"WT([A-Za-z]+)", str(dev))
        if not m or m.group(1) not in IMU_DEVICES:
            continue
        order = idx[np.argsort(t[idx], kind="stable")]
        ok = np.isfinite(t[order]) & np.isfinite(sig[order]).all(1)
        out[m.group(1)] = (t[order][ok], sig[order][ok])
    return out


def encode_imu(bufs, T=IMU_T):
    """
    A clip's IMU CSVs -> [6*D + D, T] float32: each device's acceleration and
    angular velocity resampled onto T instants spanning the clip, then one
    presence channel per device. All devices share that time axis, so arm and
    body motion stay aligned. A device with fewer than 2 rows is absent (zeros,
    presence 0). None when no device has data.
    """
    dev = {}
    for b in bufs:
        dev.update(parse_imu_csv(b))
    dev = {k: v for k, v in dev.items() if len(v[0]) >= 2}
    if not dev:
        return None
    t0 = min(v[0][0] for v in dev.values())
    t1 = max(v[0][-1] for v in dev.values())
    grid = np.linspace(t0, t1, T) if t1 > t0 else np.full(T, t0)
    D = len(IMU_DEVICES)
    x = np.zeros((7 * D, T), np.float32)
    for i, name in enumerate(IMU_DEVICES):
        if name in dev:
            tt, ss = dev[name]
            for c in range(6):
                x[6 * i + c] = np.interp(grid, tt, ss[:, c])
            x[6 * D + i] = 1.0
    return x
