...
def main():
    ...
    ap.add_argument("--center-z", action="store_true",
                    help="skeleton: also remove the root height each frame")
    ap.add_argument("--norm", choices=["clip", "imagenet", "kinetics"], default=None,
                    help="image input normalisation (default: per-clip)")
    ap.add_argument("--mixup-prob", type=float, default=None)
    ap.add_argument("--label-smoothing", type=float, default=None)
    ap.add_argument("--trunk-mult", type=float, default=None,
                    help="LR multiplier of the pretrained trunk (default 0.3)")
    ap.add_argument("--pseudo", help="npz of clip_id + label: clips the current ensemble "
                    "labels confidently join training with those labels (self-training)")
    ap.add_argument("--distill", help="npz of clip_id + probs — the ensemble's out-of-fold "
                    "probabilities for training clips and test probabilities for test clips: "
                    "train on those soft targets (knowledge distillation)")
    ap.add_argument("--distill-alpha", type=float, default=0.7,
                    help="weight of the soft-target term on clips that also have a label")
    ap.add_argument("--distill-test", action="store_true",
                    help="also train on the --distill test clips, with soft targets only")
    ...
    cfg.extra.update(temporal_aug=args.temporal_aug, amp_aug=args.amp_aug,
                     center_z=args.center_z, norm=args.norm)
    for k, v in (("mixup_prob", args.mixup_prob), ("label_smoothing", args.label_smoothing),
                 ("backbone_lr_mult", args.trunk_mult)):
        if v is not None:
            setattr(cfg, k, v)
    ...
    if args.pseudo:
        # Self-training, which the organisers allow (Kaggle topic 729989): clips the
        # ensemble labels confidently join the training set under those labels. For
        # a validation fold they are its own held-out clips, which are still scored
        # on their true labels, so the effect is measured honestly.
        ps = np.load(args.pseudo, allow_pickle=True)
        lab = dict(zip(ps["clip_id"].astype(str), ps["label"].astype(int)))
        extra = idx[idx.clip_id.astype(str).isin(lab)].copy()
        extra = extra[~extra.clip_id.isin(tr.clip_id)]
        extra["action_id"] = extra.clip_id.astype(str).map(lab).astype(int)
        tr = pd.concat([tr, extra], ignore_index=True)
        print(f"pseudo-labelled clips added to training: {len(extra)} "
              f"{extra.split.value_counts().to_dict()}")
    soft_t = hard_t = None
    if args.distill:
        # Knowledge distillation, allowed by the organisers (Kaggle topic 711665): one
        # model learns the ensemble's probabilities. Training clips keep their labels;
        # with --distill-test the test clips join with soft targets only (a soft form
        # of the test pseudo-labelling allowed in topic 729989). The loader then yields
        # row indices, and each batch looks up its labels and soft targets.
        z = np.load(args.distill, allow_pickle=True)
        teach = dict(zip(z["clip_id"].astype(str), z["probs"].astype(np.float32)))
        n_cls = int(z["probs"].shape[1])
        if args.distill_test:
            extra = test_df[test_df.clip_id.astype(str).isin(teach)].copy()
            extra = extra[~extra.clip_id.isin(tr.clip_id)]
            extra["action_id"] = -1
            tr = pd.concat([tr, extra], ignore_index=True)
        hard = tr.action_id.fillna(-1).astype(int).values
        ids = tr.clip_id.astype(str).values
        soft = np.stack([teach[c] if c in teach else np.eye(n_cls, dtype=np.float32)[max(h, 0)]
                         for c, h in zip(ids, hard)])
        print(f"distillation targets for {sum(c in teach for c in ids)}/{len(tr)} clips "
              f"({int((hard < 0).sum())} test clips with soft targets only), "
              f"alpha {args.distill_alpha}")
        hard_t = torch.as_tensor(hard, device=device)
        soft_t = torch.as_tensor(soft / soft.sum(1, keepdims=True), device=device)
        tr = tr.copy()
        tr["action_id"] = np.arange(len(tr))
    has_val = len(va) > 0
    ...
    def distill_loss(out, rows):
        """KL to the ensemble's probabilities, blended with cross-entropy on the
        label where the clip has one (rows index hard_t / soft_t)."""
        logp = torch.log_softmax(out.float(), 1)
        soft = soft_t[rows]
        kl = (soft * (torch.log(soft.clamp_min(1e-8)) - logp)).sum(1)
        hard = hard_t[rows]
        ce = nn.functional.cross_entropy(out.float(), hard.clamp_min(0), reduction="none",
                                         label_smoothing=cfg.label_smoothing)
        a = args.distill_alpha
        return torch.where(hard >= 0, (1 - a) * ce + a * kl, kl).mean()
    ...
    for ep in range(cfg.epochs):
        ...
        for x, y in train_dl:
            ...
            with torch.autocast("cuda", enabled=cfg.amp and device.type == "cuda"):
                out = model(x)
                lossf = crit if soft_t is None else distill_loss
                loss = (lam * lossf(out, ya) + (1 - lam) * lossf(out, yb)
                        if use_mix else lossf(out, y))
    ...
    # fp16 on disk (the package stores fp16 anyway): halves what gets pulled
    torch.save({k: v.half() if v.is_floating_point() else v for k, v in best_sd.items()},
               os.path.join(cfg.out_dir, f"{tag}.pt"))
